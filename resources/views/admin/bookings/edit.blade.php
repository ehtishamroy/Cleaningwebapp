...
<div class="card-body">
    <form method="POST" action="{{ route('booking.update', $booking->id) }}">
        ...
        <div class="form-group">
            ...
            <select class="form-control {{ $errors->has('duration_id') ? 'is-invalid' : '' }}" name="duration_id" id="duration_id" required>
                @foreach($durations as $duration)
                    <option value="{{ $duration->id }}" {{ $booking->duration_id == $duration->id ? 'selected' : '' }}>
                        {{ $duration->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input class="mx-1 form-check-input {{ $errors->has('review_given') ? 'is-invalid' : '' }}" type="checkbox" name="review_given" id="review_given" value="1" {{ old('review_given', $booking->review_given) ? 'checked' : '' }}>
        ...
        <div class="form-group">
            ...
            <input class="mx-1 form-check-input {{ $errors->has('is_follow_up') ? 'is-invalid' : '' }}" type="checkbox" name="is_follow_up" id="is_follow_up" value="1" {{ old('is_follow_up', $booking->is_follow_up) ? 'checked' : '' }}>
        ...
        <div class="form-group">
            ...
            <input class="mx-1 form-check-input {{ $errors->has('is_cancelled') ? 'is-invalid' : '' }}" type="checkbox" name="is_cancelled" id="is_cancelled" value="1" {{ old('is_cancelled', $booking->is_cancelled) ? 'checked' : '' }}>
